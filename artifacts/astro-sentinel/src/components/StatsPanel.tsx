...
export function StatsPanel() {
  const { data: stats, isLoading } = useGetEventStats({
    query: { queryKey: ["event-stats"], refetchInterval: 10000 }
  });
  ...
  // Normalise against different possible API shapes
  const s         = stats as any;
  const total     = s.totalEvents    ?? s.total    ?? 0;
  const rate      = s.recentRate     ?? s.rate     ?? 0;
  const byType    = s.byType         ?? {};

  const typeData = [
    { name: "GRB", value: byType.GRB ?? 0, fill: "hsl(var(--chart-1))" },
    { name: "GW",  value: byType.GW  ?? 0, fill: "hsl(var(--chart-2))" },
    { name: "FRB", value: byType.FRB ?? 0, fill: "hsl(var(--chart-3))" },
  ];
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <Card className="bg-card border-border/50 shadow-none">
        ...
        <CardContent>
          <div className="text-2xl font-bold font-mono">{Number(total).toLocaleString()}</div>
          <p className="text-xs text-muted-foreground mt-1">Global network lifetime</p>
        </CardContent>
      </Card>
      ...
      <Card className="bg-card border-border/50 shadow-none">
        ...
        <CardContent>
          <div className="text-2xl font-bold font-mono text-primary">{Number(rate).toFixed(1)}</div>
          <p className="text-xs text-muted-foreground mt-1">Events per hour (last 1h)</p>
        </CardContent>
      </Card>
